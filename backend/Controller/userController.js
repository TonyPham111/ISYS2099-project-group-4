...
export const resetPassword = async (req, res) => {
  ...
  try {
    ...
    await hrRepo.updateUserPassword(user.email, hashedPassword);
...
export const verifyEmailToken = async (req, res) => {
  ...
  try {
    const user = await hrRepo.findUserByEmailVerificationToken(token);
    if (!user || user.verificationTokenExpiry < Date.now()) {
      return res.status(400).json({ error: "Invalid or expired token." });
    }

    await hrRepo.markEmailAsVerified(user.email);

    res.status(200).json({ message: 'Email verified successfully. You can now reset your password.' });
  } catch (err) {
    console.error("Error while verifying email token:", err);
    res.status(500).json({ error: "An error occurred while verifying the email token. Please try again later." });
  }
};
...
export const requestEmailVerification = async (req, res) => {
  ...
  try {
    ...
    await hrRepo.saveEmailVerificationToken(email, verificationToken, verificationTokenExpiry);

    const verificationLink = `http://localhost:8000/user/verify-email/${verificationToken}`;
    ...
    const mailOptions = {
      ...
      text: `You are receiving this because you (or someone else) have requested to verify your email for password reset.\n\n
    ...
    console.error("Error while requesting email verification:", err);
    res.status(500).json({ error: "An error occurred while requesting email verification. Please try again later." });
  }
};
