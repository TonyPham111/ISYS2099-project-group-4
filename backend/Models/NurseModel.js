...
const nurseRepo = {
  ...
  */
  ...
  GetPatientsInfo: async (patient_name, patient_id) => {
    try {
      ...
      const [results] = await poolNurses.query(sql, [patient_name, patient_id]);
  ...
  */
  FetchPrescriptionsByPatientId: async (para_patient_id, from_date, to_date) => {
    ...
    UpdateTestDetail: async (para_patient_id, para_test_order_id, para_test_type_id, administer_staff_id, lab_result_document_id) => {
        try {
            const sql = `CALL UpdateTestDetail(?,?,?,?,?)`;
            const [results] = await poolNurses.query(sql, [para_patient_id, para_test_order_id, para_test_type_id, administer_staff_id, lab_result_document_id]);
  ...
  GetSubordinates: async (manager_id, staff_name, staff_id, sort_by, order_by) => {
    try {
      ...
      const [results] = await poolNurses.query(sql, [manager_id, staff_name, staff_id, sort_by, order_by]);
      return JSON.stringify(results, null, 2);
    } catch (error) {
      throw new Error(error.message);
    }
